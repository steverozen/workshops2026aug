...
In many cases you can ask a coding agent to adapt these plots to the data you are analyzing, but you 
still need to understand the concepts behind `ggplot`

# Example: use a coding agent to tanslate a `seaborn` example to `ggplot2`
...
the seaborn gallery: mean functional magnetic resonance
signal multiple time point per brain `region`, split by `event`.
...
:::
:::
(from <https://seaborn.pydata.org/examples/errorband_lineplots.html>)

## Prompt

```
I want to generate this plot in R using ggplot2: [Image #1], the python code is import seaborn as sns
sns.set_theme(style="darkgrid")
...
## Comments on the ggplot2 translation
...
- `theme_gray` plus the panel-background tweaks approximate seaborn's
  `darkgrid` style. 
   The remainder of the coding agent's notes were hallucinated: "If you have the `ggthemes` package,
...
Note that the python code is used several times in `seaborn` documentation and training material.
The LLM probably saw it during training.
:::
...
## Color by category + global fit --- tidyplots first try (code)

`color = state` in `tidyplot()` is inherited by **every** later layer, so a naive
`add_curve_fit()` fits **one line per state**, not the single global fit we wanted.

```{r}
#| label: bycat-tp-try-code
#| output: false
tp_try <- midwest |>
  tidyplot(x = area, y = poptotal, color = state, width = NA, height = NA) |>
  add_data_points(size = 3) |>
  add_curve_fit(method = "lm", color = "firebrick") |>            # inherits color = state -> one fit per state
...
## Color by category + global fit --- tidyplots first try (plot)

```{r}
#| label: bycat-tp-try-plot
#| echo: false
tp_try
```


## Color by category + global fit --- tidyplots fixed (code)

As we saw, `color = state` in `tidyplot()` is inherited by **every** later layer, so
`add_curve_fit()` fits one line per state. We use the `add(ggplot2::...` escape hatch.
...
## Color by category + global fit --- tidyplots fixed (plot)
...
## Grouping by color different in `ggplot2` vs `tidyplot`
