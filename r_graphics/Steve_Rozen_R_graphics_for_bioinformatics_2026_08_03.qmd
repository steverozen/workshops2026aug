...
## The same `count_kmer` in R

One function call from **stringr**.
...
Base R has `gregexpr()`, but `str_count()` reads more like the intent.

## The same `count_kmer` in Python

One method call on the string.
...
Same trick in both languages, because both use ICU/PCRE-flavoured regex.
